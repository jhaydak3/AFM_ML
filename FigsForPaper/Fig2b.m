...
numBinsContact = 1000; % Set the number of bins
...
numBinsHertzPercent = 1750; % Set the number of bins
...
for m = 1:numModels
   % Plot histogram with transparency
    ...
    % Compute and overlay KDE
     ...
     plot(xi, f*.0015, 'Color', colors(m,:), 'LineWidth', 2, 'HandleVisibility', 'off'); % Exclude KDE from legend
end
hold off;
...
for m = 1:numModels
    ...
    plot(xi, f * 1.88, 'Color', colors(m,:), 'LineWidth', 2, 'HandleVisibility', 'off'); % Exclude KDE from legend
end
hold off;

function lgraph = CNN_custom_pooling_after_lstm_3conv_relu_classification(numFeatures, sequenceLength, filterSize)
...
% No second conv block. No regression layer included.
    ...
    conv2 = convolution1dLayer(filterSize, 64, 'Padding','same', 'Name','conv2');
    ...
    conv3 = convolution1dLayer(filterSize, 128, 'Padding','same', 'Name','conv3');
    lgraph = addLayers(lgraph, conv3);

    bn3 = batchNormalizationLayer('Name','bn3');
    lgraph = addLayers(lgraph, bn3);

    relu3 = reluLayer('Name','relu3');
    lgraph = addLayers(lgraph, relu3);

    mp3 = maxPooling1dLayer(2,'Stride',2,'Name','maxpool3');
    lgraph = addLayers(lgraph, mp3);
    ...
    lgraph = connectLayers(lgraph, 'maxpool2','conv3');
    lgraph = connectLayers(lgraph, 'conv3','bn3');
    lgraph = connectLayers(lgraph, 'bn3','relu3');
    lgraph = connectLayers(lgraph, 'relu3','maxpool3');
    
    lgraph = connectLayers(lgraph, 'maxpool3','lstmSeq');

function evaluate_model_classification_CNN_custom_v2(layers, preprocessedDataFile, saveName, useAllFeatures)
...
for foldID = 1:k
    ...
    % ----------------------------
    % Convert from 3D -> 2D for SMOTE
    XTrain2D = reshape(XTrain, [], size(XTrain,3))';
    % XTrain is [numFeatures x numPoints x numSamples] => flatten features & transpose => [numSamples x (features*numPoints)]

    % Convert YTrain from categorical to numeric
    YTrainNum = zeros(size(YTrain));
    YTrainNum(YTrain=='reject') = 1;  % or 0
    YTrainNum(YTrain=='accept') = 2;  % or 1

    % Apply SMOTE
    [X2D_SMOTE, YNum_SMOTE] = smote(XTrain2D, [], 5, 'Class', YTrainNum);

    % Reshape back to 3D
    numSamplesAug = size(X2D_SMOTE,1);
    XTrainAug = reshape(X2D_SMOTE', [size(XTrain,1), size(XTrain,2), numSamplesAug]);

    % Convert numeric labels back to categorical
    YTrainAug = categorical(YNum_SMOTE, [1,2], {'reject','accept'});

    % -----------
    % Then proceed with training your CNN
    % -----------
    ...
    accuracyTrainFold = mean(string(YPredTrainLabels)==string(YTrainAug));
